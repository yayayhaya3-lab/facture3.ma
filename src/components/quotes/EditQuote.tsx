...
import { useData, Client, QuoteItem, Quote } from '../../contexts/DataContext';
...
export default function EditQuote({ quote, onSave, onCancel }: EditQuoteProps) {
  const { user } = useAuth();
  const { t } = useLanguage();
  const { clients, products, getClientById } = useData();
  
  const [quoteDate, setQuoteDate] = useState(quote.date);
  const [validUntil, setValidUntil] = useState(quote.validUntil);
  const [selectedClientId, setSelectedClientId] = useState(quote.clientId);
  const [selectedClient, setSelectedClient] = useState<Client | null>(quote.client);
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className="lg:col-span-2 space-y-6">
                ...
                <div className="bg-gray-50 rounded-xl p-6">
                  <h4 className="text-md font-semibold text-gray-900 mb-4">Informations Devis</h4>
                  ...
                  {products.length === 0 && (
                    <div className="bg-amber-50 border border-amber-200 rounded-lg p-4 mb-4">
                      <p className="text-amber-800 text-sm">
                        ⚠️ Aucun produit enregistré. Veuillez d'abord ajouter des produits dans la section Produits pour pouvoir modifier un devis.
                      </p>
                    </div>
                  )}
                ...
                <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6">
                  ...
                  <div className="mt-6 p-4 bg-purple-50 rounded-lg">
                    <p className="text-xs text-purple-800">
                      Ce devis sera valable jusqu'au {new Date(validUntil).toLocaleDateString('fr-FR')} et pourra être converti en facture.
                    </p>
                  </div>
                </div>
              </div>
